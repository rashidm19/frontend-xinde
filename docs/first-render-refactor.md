...
### Segment Groups
- pp/[locale]/(public) contains pages that do not require authentication.
- pp/[locale]/(protected) wraps all authenticated routes with a server-side guard.
- Segment groups keep URLs unchanged while ensuring the correct layout and guard are automatically applied.

### Server Guard Flow
1. Incoming request hits the locale middleware which prefixes the path and preserves method/queries.
2. Protected routes render through (protected)/layout.tsx, a Server Component.
3. The layout calls getMe, getSubscription, and getBalance on the server, prefetches them into a QueryClient, then dehydrates the cache.
4. If no user is returned, the layout issues an SSR redirect to /{locale}/login, preventing any client-side flash of unauthenticated UI.
...
- React Query defaults (staleTime 5m, gcTime 30m, no auto-refetch on mount/focus/reconnect, retry=1) live in lib/queryClient.ts.
...
1. Place the page under pp/[locale]/(protected).
...
- Place the page under pp/[locale]/(public) when no authentication is required.
...
- Use Providers at the root (pp/[locale]/layout.tsx) and HydrateOnly inside (protected)/layout.tsx.
