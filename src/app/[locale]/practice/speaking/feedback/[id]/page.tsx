  ...
  return (
    <div className='flex min-h-[100dvh] items-center justify-center bg-d-red-secondary px-[24rem]'>
      <div className='flex max-w-[480rem] flex-col items-center gap-[24rem] rounded-[28rem] border border-dashed border-slate-200 bg-white px-[40rem] py-[48rem] text-center shadow-[0_24rem_80rem_-64rem_rgba(46,67,139,0.35)]'>
        ...
        >
          <p className='text-[15rem] text-slate-600'>Our AI is analyzing your IELTS performance using official scoring criteria.</p>
          <p className='text-[15rem] text-slate-600'>You can wait here or come back to view your feedback later.</p>
        </motion.div>
        <StatusMarquee reducedMotion={!!shouldReduceMotion} />
        <ActionButtons reducedMotion={!!shouldReduceMotion} onGoToProfile={() => router.push('/profile')} />
      </div>
    </div>
  );
}
...
const STATUS_MESSAGES = [
  'Checking pronunciation clarity…',
  'Assessing lexical range…',
  'Analyzing coherence and fluency…',
  'Comparing against IELTS descriptors…',
];

function StatusMarquee({ reducedMotion }: StatusMarqueeProps) {
...
function ActionButtons({ reducedMotion, onGoToProfile }: ActionButtonsProps) {
  return (
    <motion.div
