        ...
        {isExpanded && (
          ...
          >
            <div className='space-y-[12rem] border-t border-[#E1D6B4]/40 px-[16rem] py-[14rem]'>
              ...
              {question.hint ? (
                <div>
                  <p className='text-[11rem] font-semibold uppercase tracking-[0.15em] text-[#85784A]'>Hint</p>
                  <p className='mt-[4rem] text-[13rem] leading-[1.6] text-d-black/70'>{question.hint}</p>
                </div>
              ) : null}

              {question.picture ? (
                <div>
                  <p className='text-[11rem] font-semibold uppercase tracking-[0.15em] text-[#85784A]'>Image</p>
                  <img src={question.picture} alt={`Question ${question.number} illustration`} className='mt-[8rem] max-h-[200rem] rounded-[8rem] object-contain' />
                </div>
              ) : null}

              {question.x !== undefined && question.y !== undefined ? (
                <div>
                  <p className='text-[11rem] font-semibold uppercase tracking-[0.15em] text-[#85784A]'>Table Position</p>
                  <p className='mt-[4rem] text-[13rem] text-d-black/70'>
                    Row {question.y + 1}, Column {question.x + 1}
                  </p>
                </div>
              ) : null}
              ...
              {status !== 'correct' && (
                <p className='text-[12rem] italic text-[#6F6335]/80'>
                  {status === 'unanswered' ? 'Try to answer every question next time for better practice.' : 'Review the audio script to understand why this answer is correct.'}
                </p>
              )}
            </div>
          </motion.div>
...
function formatAnswer(answer: string | null, block: ListeningBlockFeedback): string {
  if (!answer) return '';

  if ((block.kind === 'titles' || block.kind === 'matching') && 'choices' in block) {
    const choice = block.choices.find(c => c.answer === answer);
    if (choice) {
  ...
  if (block.kind === 'test' && 'questions' in block) {
    // For test questions, find the question and its choices
    const q = block.questions.find(question => {
      if (question.user_answer === answer || question.correct_answer === answer) return true;
      return false;
    });
    if (q?.choices) {
      const choice = q.choices.find(c => c.answer === answer);
      if (choice) {
        return `${answer}. ${choice.choice}`;
      }
    }
