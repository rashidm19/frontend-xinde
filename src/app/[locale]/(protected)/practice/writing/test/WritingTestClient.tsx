...
type WritingTestClientProps = {
  practiceId: string;
};

function WritingTestClient({ practiceId }: WritingTestClientProps) {
  ...
  const { data, status } = useQuery({
    queryKey: ['practice-writing', practiceId],
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (response?.id) {
        await clearPracticeSessionCookie('writing');
...
const HydratedWritingTest = withHydrationGuard(WritingTestClient);

export default HydratedWritingTest;
