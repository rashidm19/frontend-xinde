...
interface ReadingBandMappingModalProps {
  open: boolean;
...
function ReadingBandMappingModalComponent({ open, onOpenChange, bandMapping, correctCount, shouldReduceMotion }: ReadingBandMappingModalProps) {
  const highlightedRange = useMemo(() => {
    return bandMapping.find(entry => correctCount >= entry.minCorrect && correctCount <= entry.maxCorrect) ?? null;
  }, [bandMapping, correctCount]);
              ...
              <DialogPrimitive.Content asChild>
                <div className='fixed inset-0 z-[2001] flex items-end justify-center px-[12rem] py-[16rem] tablet:items-center tablet:px-[32rem] tablet:py-[40rem]'>
                  <motion.div
                    ...
                    className='relative w-full max-w-[720rem] overflow-hidden rounded-t-[32rem] border border-[#E1D6B4] bg-white shadow-[0_32rem_80rem_-42rem_rgba(56,56,56,0.26)] tablet:max-w-[880rem] tablet:rounded-[32rem]'
                  >
                    <header className='flex items-center justify-between gap-[12rem] border-b border-[#E1D6B4] bg-d-yellow-secondary/70 px-[24rem] py-[18rem]'>
                      <div className='flex flex-col gap-[4rem]'>
                        <DialogPrimitive.Title className='text-[18rem] font-semibold text-d-black'>Reading band mapping</DialogPrimitive.Title>
                        <DialogPrimitive.Description className='text-[13rem] text-d-black/70'>
                          Understand how your correct answers translate into an estimated IELTS band.
                    ...
                    <div className='relative max-h-[70vh] overflow-hidden'>
                      ...
                      <div className='max-h-[70vh] overflow-y-auto px-[24rem] py-[24rem] scrollbar-thin scrollbar-thumb-[#E1D6B4]'>
                        <table className='w-full table-auto border-separate border-spacing-y-[8rem] text-left'>
                          <thead>
                            <tr className='text-[13rem] font-semibold uppercase tracking-[0.16em] text-[#85784A]'>
                              <th scope='col' className='rounded-l-[18rem] bg-d-yellow-secondary/70 px-[18rem] py-[12rem]'>
                                Correct answers
                              </th>
                              <th scope='col' className='rounded-r-[18rem] bg-d-yellow-secondary/70 px-[18rem] py-[12rem]'>
                                Estimated band
                              </th>
                            </tr>
                          </thead>
                          <tbody className='text-[14rem] text-d-black'>
                              ...
                              return (
                                <tr
                                  ...
                                  className={cn(
                                    ...
                                    isActive ? 'border-[#C9E0B7] bg-[#E7F2DD] shadow-[0_16rem_32rem_-28rem_rgba(60,111,44,0.28)]' : 'bg-white'
                                  )}
                                >
                                  <td className='rounded-l-[18rem] px-[18rem] py-[12rem] font-semibold'>{rangeLabel}</td>
                                  <td className='rounded-r-[18rem] px-[18rem] py-[12rem] text-[15rem] font-semibold text-d-black'>{entry.estimatedBand}</td>
                                </tr>
                              );
                            })}
                          </tbody>
        ...
        <div className='flex min-h-0 flex-1 flex-col overflow-hidden rounded-t-[28rem] bg-white/95 pb-[calc(16rem+env(safe-area-inset-bottom))] shadow-[0_-18rem_36rem_-28rem_rgba(15,23,42,0.22)] backdrop-blur-lg'>
          <header className='sticky top-0 z-[2] flex items-center justify-between gap-[12rem] border-b border-[#E1D6B4] bg-white/95 px-[20rem] py-[18rem]'>
            <div className='flex flex-col gap-[2rem] text-left'>
              <span className='text-[12rem] font-semibold uppercase tracking-[0.18em] text-[#85784A]'>Performance</span>
              <h2 className='text-[18rem] font-semibold text-d-black'>Reading band mapping</h2>
              <p className='text-[13rem] text-d-black/70'>Understand how your correct answers translate into an estimated IELTS band.</p>
            </div>
...
export const ReadingBandMappingModal = withHydrationGuard(ReadingBandMappingModalComponent);
