...
const CarouselContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  ...
  return (
    <div ref={carouselRef} className='overflow-hidden'>
      <div ref={ref} className={cn('flex', orientation === 'horizontal' ? '-ml-[20rem]' : '-mt-[20rem] flex-col', className)} {...props} />
    </div>
  );
});
...
const CarouselItem = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn('min-w-0 shrink-0 grow-0 basis-full', orientation === 'horizontal' ? 'pl-[20rem]' : 'pt-[20rem]', className)}
      {...props}
    />
  );
});
