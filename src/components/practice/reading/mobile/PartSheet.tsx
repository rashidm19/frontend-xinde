...
export const PartSheet: React.FC<PartSheetProps> = ({ open, onOpenChange, options, activeId, onSelect, title, subtitle, closeLabel }) => {
  ...
  return (
    <BottomSheet open={open} onOpenChange={onOpenChange}>
      <BottomSheetContent className="px-[20rem] pb-[24rem] pt-[20rem]" aria-label={title} role="dialog" aria-modal="true">
            ...
            {options.map(option => {
              ...
              return (
                <motion.button
                  ...
                  {...sheetVariants}
                  className={cn(
                    "flex flex-col items-start gap-[6rem] rounded-[18rem] border border-transparent bg-white px-[18rem] py-[16rem] text-left shadow-[0_12rem_32rem_rgba(56,56,56,0.08)] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-d-green/50",
